...
public class ProgressBarPro : MonoBehaviour
{
    ...
    public void ReloadValue(float percentage,float duration, bool forceUpdate = false)
    {
        ...
        if (animateBar && Application.isPlaying && gameObject.activeInHierarchy)
            StartSizeAnim(percentage, duration);
        else
    ...
    public void SetValue(float percentage, bool forceUpdate = false)
    {
        ...
        if (animateBar && Application.isPlaying && gameObject.activeInHierarchy)
            StartSizeAnim(percentage, animTime);
        else
            SetDisplayValue(percentage);
    }
    ...
    void StartSizeAnim(float percentage, float _time)
    {
        if (sizeAnim != null)
            StopCoroutine(sizeAnim);

        sizeAnim = StartCoroutine(DoBarSizeAnim(_time));
    }

    IEnumerator DoBarSizeAnim(float _time)
    {
        float startValue = displayValue;
        float time = 0f;
        float change = m_value - displayValue;
        float duration = (animationType == AnimationType.FixedTimeForChange ? _time : Mathf.Abs(change) / _time);
